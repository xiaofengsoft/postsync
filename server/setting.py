...
def save():
    ...
    data['app']['debug'] = str(data['app']['debug']).strip("'").lower() == 'true'
    data['default']['headless'] = str(data['default']['headless']).strip("'").lower() == 'true'
    data['default']['devtools'] = str(data['default']['devtools']).strip("'").lower() == 'true'
    data['default']['no_viewport'] = str(data['default']['no_viewport']).strip("'").lower() == 'true'
    with open(get_root_path() + '/config.yaml', 'w', encoding='utf-8') as file:
        yaml.dump(data, file, default_flow_style=False, encoding='utf-8',Dumper=SafeDumper,sort_keys=False,allow_unicode=True)
