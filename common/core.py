...
def create_default_sync_context(url="") -> tuple:
    """
    同步创建默认的浏览器上下文，包括浏览器和页面对象
    之后需要手动关闭sp和browser
    :param url:
    :return: browser 浏览器对象, page 当前页面对象, p Playwright对象 , sp PlaywrightContextManager对象
    """
    sp = sync_playwright()
    p = sp.__enter__()
    ...
    return browser, page, p, sp



def sync_post_files(files: list, digests=None, sites=None, tags=None, categories=None, covers=None,
        ...
        with open(file, 'r', encoding='utf-8') as f:
            ...
            for site in sites:
                ...
                if site not in config['default']['community']:
                    continue
                result.append(sync_post_text(file, title, content, digests[index], site, tags[index], categories[index],
                                             covers[index],
                                             topics[index], columns[index]))

    # TODO 上传后的链接
    return Result(code=1, message='success', data=result)
...
async def async_post_files(files: list, digests=None, sites=None, tags=None, categories=None, covers=None,
    ...
    browser, page, ap, asp = await create_default_async_context()
    tasks = []
    for (index, file) in enumerate(files):
        (title, ext) = get_file_name_ext(file)
        if ext == '.md' or ext == '.markdown' or ext == '.mdown' or ext == '':
        ...
        with open(file, 'r', encoding='utf-8') as f:
            ...
            for site in sites:
                ...
                task = async_post_text(page, file, title, content, digests[index], site, tags[index], categories[index],
                                       covers[index],
                                       topics[index], columns[index])
                tasks.append(task)
...
async def async_post_text(page: object, file_path: str, title: str, content: str, digest: str | object = None,
                          site: str = None,
                          tags: list = None,
    ...
    site_instance = site_instance(page)
